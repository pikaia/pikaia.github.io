...
| "Tsao" (the Tsao Foundation) | Unknown word | Spelled out letter by letter — no lexicon entry (Chinese surname 曹) | `tsˈW` ("TSOW", rhymes with "how"/"now"). **Not ear-verified yet** — three samples synthesized ("tsow" / "chow" / "zow"); flag if it sounds off | `PRONUNCIATION_OVERRIDES` | the-ageism-gap-singapore-employers |
