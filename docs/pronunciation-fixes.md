...
| Tiong Bahru / SIT post: "hoc" ("ad hoc"), the architect surname "Kan", "prewar", "dockworkers", the Hokkien nickname fragments "puay"/"chu" (*puay kee chu*, "aeroplane house") and "mei" (*mei ren wo*, "den of beauties") — **plus the post's own subject name "Tiong"** (as in Tiong Bahru, used 12+ times) | Unknown word | Spelled out letter by letter | `hˈɒk` / `kˈɑːn` ("kahn") / `pɹˈiːwɔː` ("PREE-wor") / `dˈɒkwˈɜːkəz` / `ʧˈuː` ("choo", matches this dict's existing "Choo") / `mˈA` ("may") — all single-obvious-reading, no sample needed. `pwˈA` ("pway") for "puay" is a best guess, **NOT ear-verified yet** (single-mention colloquial nickname, low stakes). `tjˈɒŋ` ("tyong", literal spelling with a y-glide — candidate 2 of 3) picked by Chris by ear (2026-09-12) for "Tiong" itself, over `ʧˈɒŋ` ("chong", the anglicized Hokkien-glide reading a web summary of local usage had suggested) and `tiːˈɒŋ` ("tee-ong", fully spelled), from samples in `scratch/before-hdb-singapore-improvement-trust-tiong-bahru-pronunciation/` | `PRONUNCIATION_OVERRIDES` | before-hdb-singapore-improvement-trust-tiong-bahru |
