...
| Malay / Chinese kampong names on Pulau Tekong: "Yong", "Kong" (San Yong Kong), "Batu", "Koyok", "Merah", "Sungei", "Belang", "Ayer", "Samak", "Pengkalan", "Pakau" | Unknown word | All spelled out letter by letter — no lexicon entries | `jˈɒŋ` / `kˈɒŋ` / `bˈɑːtuː` / `kˈQjɒk` / `mˈɛrɑː` / `sˈʊŋaɪ` / `bˈɛlaŋ` / `ˈaɪə` / `sˈɑːmak` / `pəŋkˈɑːlan` / `pˈɑːkaʊ` — anglicized approximations, same batch pattern as the National Service post's Malay place names. Two grouped samples synthesised for ear review; "San Yong Kong" is three words here so "Yong"/"Kong" get their own entries even though the NS post has "Sanyongkong" as one word | `PRONUNCIATION_OVERRIDES` | kampongs-under-pulau-tekong |
