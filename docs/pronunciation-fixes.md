...
| "reliably" | Wrong entry / homograph collision | misaki gives "reliable" and "reliability" the correct "ry" (/aɪ/) vowel, but "reliably" specifically gets a short-ɪ instead, dropping the "-LY-" sound entirely - heard by Chris as the word coming out garbled, "like the speaker was tongue tied" | `ɹɪlˈIəbli` - "reliable"'s own correct vowel (`ɹɪlˈIəbᵊl`) with the "-ly" ending swapped in, replacing the wrong `ɹɪlˈɪəbli` misaki produces on its own. Confirmed directly via `pipe.g2p()` (not guessed) - see chat, same root-cause class as "stung"/"graves"/"mall" above | `PRONUNCIATION_OVERRIDES` | cold-storage-and-the-frozen-meat-trade |
