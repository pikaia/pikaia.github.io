...
| Pan-Electric 1985 post: "Koon" (Tan Koon Swan, ×5), "Tham" (Peter Tham), "Kahlenberg" (the founder), "Arang" (Kampong Arang Road), "Selco"/"Acma" (subsidiaries), "Sek"/"Keong" (Chan Sek Keong), "Frimatic" (defunct French fridge brand) — **plus "diversified"** (plain English, no misaki entry); "CLOB" said as a word; "OCBC"/"OUB" letter-spelled | Unknown word | Spelled out letter by letter; "CLOB" too (should be a word) | `kˈuːn` / `tˈɑːm` ("tarm", rhymes with "calm") / `kˈɑːlənbɜːɡ` / `ˈɑːraŋ` / `sˈɛlkəʊ` / `ˈakmə` / `sˈɛk` / `kiˈɒŋ` ("kee-ONG") / `frˈɪmatɪk` / `daɪvˈɜːsɪfaɪd`. `CLOB` → `klˈɒb` ("clob", rhymes with "job", per Chris) — Central Limit Order Book, said as a word not spelled out. "OCBC"/"OUB" added to `KNOWN_LETTER_SPELLED` (bank initialisms, always spelled out). "Frimatic" `frˈɪmatɪk` ("FRIH-mat-ic") picked by Chris by ear (2026-09-02) from 3 samples in `scratch/three-days-stock-exchange-shut-1985-frimatic/`, over "FRY-mat-ic" `frˈaɪmatɪk` and "free-MAT-ic" `friːmˈatɪk`; the rest are single-obvious-reading | `PRONUNCIATION_OVERRIDES` + `KNOWN_LETTER_SPELLED` | three-days-stock-exchange-shut-1985 |
| Jalan Payoh Lai / Kangkar post: this post's narration predates the fix process, so its full `--dry-run` flagged **27 sentences at once**. Teochew/Malay place names of old Hougang ("Jalan", "Payoh"/"Lai", "Kangkar"/"kangkars"/"kangchu", "Hougang", "Aukang"/"Au", "Punggol", "Wak"/"Sumang"), a French priest and saint ("Ambroise"/"Maistre", "Marie"/"Grignion"/"de"), the school name "Montfort" (×9) — **plus two ordinary compound-word gaps** ("backlane", "Neo-Gothic"); "St" (no period) before a capitalised name text-expanded to "Saint" | Unknown word / compound gap | Spelled out letter by letter; "St" → "S, T" | `dʒˈalan` (Wiktionary Malay /ˈdʒalan/) / `pɑːjˈəʊ` / `lˈaɪ` / `kˈʌŋkɑː` + `kˈʌŋkɑːz` ("KUNG-kah" — Chris picked this by ear over the English-analogy "kang-KAH", which sounded Western; first syllable rhymes with "Kung") / `kˈaŋtʃuː` ("KANG-choo" — deliberately *not* matched to Kangkar's "KUNG-", even though both are 港: per Chris, an academic term taken from written Mandarin stays near Mandarin, while a name the local population absorbed picks up the vernacular shift, so it's fine for the less-common word to sound different) / `hˈaʊɡaŋ` / `ˈaʊkaŋ` / `aʊ` / `pˈɒŋɡəʊl` (Wikipedia /ˈpɒŋɡoʊl/) / `wˈɑːk` / `sˈuːmaŋ` / `ˈambrwɑːz` / `mˈɛstrə` / `mərˈiː` / `ɡrˈiːnjɒn` (French /ɡʁiɲɔ̃/) / `də` / `mˈɒntfət` / `bˈaklAn` / `nˌiːəʊɡˈɒθɪk`. `re.compile(r"\bSt(?=\s+[A-Z][a-z])")` → "Saint" (narrow — won't touch "21st" or "St." with a period, per the ambiguous-"St." policy note above). Most flagged locals are places **Chris grew up in** (born 1960 on Jalan Payoh Lai); he confirmed every lead by ear (2026-09-03) from samples in `scratch/jalan-payoh-lai-kangkar-montfort-nativity-church-pronunciation/` ("ur default choices are excellent"), no corrections. Jalan / Hougang / Punggol recur across many Singapore posts | `PRONUNCIATION_OVERRIDES` + `ABBREVIATION_EXPANSIONS` | jalan-payoh-lai-kangkar-montfort-nativity-church |
| Pineapple-kings post: the three towkays and old place/person names misaki spells out — "Kah" (Tan Kah Kee), "Tye" (Tan Tye), "Sembawang", "Yishun", "Skudai", "Bastiani" (the Corsican-Italian cannery owner), "Yat-sen", "Kempeitai"/"Kempeitai's", "Xiamen" (Xiamen University) | Unknown word | Spelled out letter by letter | `kˈɑː` / `tˈaɪ` / `sɛmbˈawaŋ` ("sem-BAH-wang") / `jˈiːʃʊn` ("YEE-shun") / `skˈuːdaɪ` ("SKOO-dye") / `bˌastiˈɑːni` / `jˌatsˈɛn` / `kˌɛnpeɪtˈaɪ` + `kˌɛnpeɪtˈaɪz` ("ken-pay-TIE") / `ʃjˈɑːmˌɛːn` ("SHYAH-mehn"). Sembawang/Yishun/Skudai confirmed by Chris by ear (2026-09-04) from samples in `scratch/singapore-canned-pineapple-kings-pronunciation/`. Kempeitai is the interesting one: Chris corrected all three samples' "kem" to "ken" by ear, then — per his own standing rule below ("for words with racial/ethnic roots, default to the origin language's actual pronunciation") — we checked the Japanese: ん assimilates to [m] before a bilabial (p/b/m), which is exactly why the older English military-history spelling is "Kempeitai" rather than modern Hepburn's "Kenpeitai"; "kem" is arguably the more phonetically faithful reading. Chris's explicit final call was still "ken". Xiamen was caught later, during video review, not the original `--dry-run` batch: the default reading came out as "char-men"/"shaa-muhn", and round-1 candidates (plain `mˈɛn`) all had the second syllable rushed regardless of stress placement — **a plain short vowel on the final syllable of a word can get clipped by Kokoro's timing independent of stress; add a length mark (`ː`) to that vowel to fix it**, same technique as "Laba" above. Confirmed by Chris by ear (2026-09-04) from 2 round-2 samples. "Kong"/"Kee"/"Nee"/"Soon"/"Rochor" already covered. Sembawang / Yishun / Kempeitai recur across many Singapore posts | `PRONUNCIATION_OVERRIDES` | singapore-canned-pineapple-kings |

| Wild-tigers post ("When Singapore Had Wild Tigers"): old place names and the 1928/1930 hunting party's names — "Choa" (Choa Chu Kang), "Rangong" (Rangong Road, the old Serangoon Road), "Tian"/"Quee" (Tan Tian Quee), "Mui" (Tan Ah Mui), "Mahally" (the tracker), "Ong" (Ong Kim Hong), "Buloh" (Sungei Buloh), "Ubin" (Pulau Ubin) — **plus a batch of plain English / anglicised gaps**: "Billiard", "McGowan", "McNair", "Robertson", "recolonised", "Saltwater", "euthanised" | Unknown word | Spelled out letter by letter | `tʃwˈɑː` ("chwah", one syllable) / `raŋɡˈɒŋ` ("rang-GONG" — the root of "Serangoon" `sˌɛrəŋɡˈuːn`, already covered) / `tjˈɛn` ("tyen") / `kwˈiː` ("kwee") / `mˈuːi` ("MOO-ee") / `məhˈɑːli` ("muh-HAH-lee" — origin unclear, best-guess reading) / `ˈɒŋ` ("ong", rhymes with "song") / `bˈuːloʊ` ("BOO-loh", Malay "bamboo", first-syllable stress) / `ˈuːbɪn` ("OO-bin", Malay "tile") / `bˈɪljəd` / `məɡˈaʊən` / `məknˈɛə` / `rˈɒbətsən` / `rˌiːkˈɒlənaɪzd` / `sˈɔːltwɔːtə` / `jˈuːθənaɪzd`. The six local names (Choa, Tian, Quee, Mui, Mahally, Buloh) confirmed by Chris by ear (2026-09-05) from samples in `scratch/when-singapore-had-wild-tigers-pronunciation/` — he picked candidate 1 for all six, no corrections. "Kang"/"Chu"/"Bukit"/"Timah"/"Serangoon"/"Sungei"/"Pulau"/"Sembawang" already covered. Choa / Ubin / Buloh recur across Singapore posts | `PRONUNCIATION_OVERRIDES` | when-singapore-had-wild-tigers |
| Coney Island post ("Coney Island, Before It Was a Nature Park"): the 1950 buyer's name "Ghulam"/"Mahmood" (Urdu/Arabic origin; he was a local Indian/Muslim businessman) — **plus three plain English gaps**: "Ferris" (Ferris wheel), "verandah", "wetter" (comparative of "wet"); and "BBC" letter-spelled | Unknown word | Spelled out letter by letter | `ɡuˈlɑːm` ("ghu-LAHM", hard g, stress 2nd, long aa — closest to the Urdu) / `mɑːˈmuːd` ("mah-MOOD", stress 2nd — closest to the Arabic) / `fˈɛrɪs` / `vərˈandə` / `wˈɛtər`. Ghulam + Mahmood picked by Chris by ear (2026-09-08) from samples in `scratch/coney-island-before-it-was-a-nature-park-pronunciation/` — candidate 1 (the origin-language reading) for both, over the anglicised first-syllable-stress alternatives. "BBC" added to `KNOWN_LETTER_SPELLED` — read letter by letter, misaki's default is correct. "Punggol"/"Serangoon" already covered | `PRONUNCIATION_OVERRIDES` + `KNOWN_LETTER_SPELLED` | coney-island-before-it-was-a-nature-park |
| Rickshaw men post ("The Rickshaw Men and the Jinricksha Station"): the two Fujian dialect-group names "Hokchew"/"Henghua" and the Hokkien term "keng" (in "coolie keng") — **plus plain English / name gaps**: "capped", "brickwork", "signage", "tableau", and the architects "Tomlinson"/"McLeod"/"Craik" | Unknown word | Spelled out letter by letter | `hˈɒktʃuː` ("HOK-choo") / `hˈɛŋhwɑː` ("HENG-hwah") / `kˈɛŋ` ("keng", rhymes with "ten"+ng) / `kˈapt` (same gap as "tapped"/"mapped") / `brˈɪkwɜːk` / `sˈaɪnɪdʒ` / `tˈabləʊ` / `tˈɒmlɪnsən` / `məklˈaʊd` (**"muh-CLOUD"**, the Scottish name — not "mac-LEE-odd") / `krˈeɪk` ("krayk", rhymes with "cake"). Hokchew / Henghua / keng picked by Chris by ear from 2–3 samples each in `scratch/the-rickshaw-men-and-the-jinricksha-station-pronunciation/`; the rest are single-obvious-reading | `PRONUNCIATION_OVERRIDES` | the-rickshaw-men-and-the-jinricksha-station |
| Kallang Airport post ("Kallang Airport, 'the finest airport in the British Empire'"): "Kallang" (×20), "Paya"/"Lebar" (Paya Lebar), the Malay word "merdeka", the aviator "Amelia"/"Earhart", the airline "Wearne's", the architect "Dorrington" (Frank Dorrington Ward) — **plus plain English / name gaps**: "hemmed", "seawall", "Lockheed", "façade"; "KLM"/"KNILM" letter-spelled | Unknown word | Spelled out letter by letter | `kˈɑːlaŋ` ("KAH-lang", first-syllable stress) / `pˈɑːjɑː` ("PAH-yah") / `lˈɛbɑː` ("LEH-bar") / `mɜːdˈeɪkə` ("mer-DAY-ka") / `ɐmˈiːliɐ` / `ˈɛəhɑːt` ("AIR-hart"; also covers "Earhart's") / `wˈɜːnz` ("WERNZ") / `dˈɒɹɪŋtən` ("DOR-ing-tun") / `hˈɛmd` / `sˈiːwɔːl` / `lˈɒkhiːd` / `fəsˈɑːd` ("fuh-SAHD"; the c-cedilla blocks misaki). Kallang / merdeka / Paya Lebar / Earhart / Wearne's all picked by Chris by ear (2026-09-09) from 2–3 samples each in `scratch/kallang-airport-finest-in-the-british-empire-pronunciation/` — candidate 2 for Kallang, 1 for merdeka, A for Paya Lebar, 1 for Earhart and Wearne's; the rest are single-obvious-reading. "KLM"/"KNILM" added to `KNOWN_LETTER_SPELLED` — read letter by letter, misaki's default is already correct ("BOAC" too, but misaki spells that one without prompting). Also: the post's front-matter `title` had escaped inner double-quotes (`\"…\"`) that leaked literal backslashes into the narration — switched to single inner quotes, the house style (cf. the Christmas Island title) | `PRONUNCIATION_OVERRIDES` + `KNOWN_LETTER_SPELLED` | kallang-airport-finest-in-the-british-empire |
| Car-plants post ("Assembled in Singapore: The Short Life of Local Car Plants"): car marques and place/person names — "Hyundai"/"Hyundai's", "Ioniq", "Nissan", "Mitsubishi", "Mercedes"/"Mercedes-Benz", "Daihatsu", "Isuzu", "Volvo", "Studebaker", "Wearne" (Wearne Brothers), "Percival", "Émile"/"Brizay" (the Ford plant's architect), "Enggor" (Enggor Street), "Hillview" — **plus plain English gaps** "industrialisation", "robotaxi", "café" | Unknown word | Spelled out letter by letter | `hˈʌndeɪ` ("HUN-day", like Sunday) + `hˈʌndeɪz` / `aɪˈɒnɪk` ("eye-ON-ik", like "ionic") / `nˈɪsan` / `mˌɪtsʊbˈiːʃi` / `məsˈeɪdiːz` + `məsˌeɪdiːzbˈɛnz` / `daɪhˈatsuː` / `ɪsˈuːzuː` / `vˈɒlvəʊ` / `stˈuːdəbeɪkə` / `wˈɜːn` (matches "Wearne's" `wˈɜːnz` from the Kallang batch) / `pˈɜːsɪvəl` / `eɪmˈiːl` + `brɪzˈeɪ` / `ˈɛŋɡɔː` / `hˈɪlvjuː` / `ɪndˌʌstriəlaɪzˈeɪʃən` / `rˈəʊbəʊtˌaksi` / `kˈafeɪ`. Hyundai and Ioniq picked by Chris by ear (2026-09-10) from 3 samples each in `scratch/assembled-in-singapore-local-car-plants-pronunciation/` — candidate 1 for both; the rest are single-obvious-reading. **Process note:** the first `1.2` narration run was launched before these overrides were added, so its audio spelled the names out. The fix was a plain re-run of `1.2` (no `--no-cache` needed — `_sentence_cache_key` already folds in each sentence's relevant overrides, so only the 17 affected sentences re-synthesised). Still, add pronunciation overrides *before* narration synth, not alongside it | `PRONUNCIATION_OVERRIDES` | assembled-in-singapore-local-car-plants |
| Johor–Singapore Causeway post: "Tuas" (the district / Second Link, ×3), the engineers "Coode"/"Fitzmaurice"/"Wilson"/"Topham"/"Railton", the regiment "Argyll", and "Gelang"/"Patah" (Gelang Patah, Johor) | Unknown word | Spelled out letter by letter | `twˈɑːs` ("twahs", one syllable — Chris ear-picked candidate 3 of 3, 2026-09-11, over the two-syllable "TOO-ass"/"TOO-ahs" readings) / `kˈuːd` ("cood", rhymes with "food") / `fɪtsmˈɒrɪs` / `wˈɪlsən` / `tˈɒpəm` / `rˈeɪltən` / `ɑːɡˈaɪl` ("ar-GILE", stress 2nd — **not** "AR-gill") / `ɡəlˈaŋ` + `pˈatɑː` (distinct from Singapore's "Geylang" `ɡˈeɪlaŋ`, already in the dict). Only Tuas needed a sample (`scratch/johor-singapore-causeway-pronunciation/`); the rest are single-obvious-reading | `PRONUNCIATION_OVERRIDES` | johor-singapore-causeway |
| Nutmeg/Orchard Road post ("The Nutmeg Estates That Became Orchard Road"): the planters and place names "Oxley"/"Cairnhill"/"Cuppage"/"Killiney"/"Bencoolen"/"Tanglin"/"Burkill"/"Niven"/"Koek"/"Seah"/"Kiat", the artist "Kumari"/"Nahappan", the architect "Rajoo", the Malay words "Balik"/"Peranakan", and "Geok" (Kwa Geok Choo) — **plus plain English gaps** "Lawrence", "Edwin", "businessmen" (misaki has "businessman" but not the plural), "Choo", and the hyphenated compound "Agri-Horticultural" (misaki has "agriculture"/"Horticultural" alone but not this compound as one token) | Unknown word | Spelled out letter by letter | `ˈɒksli` ("OX-lee") / `kˈɛːnhɪl` / `bɛnˈkuːlən` ("ben-COOL-un" — already a Singapore place name) / `tˈaŋlɪn` / `ˈlɒrəns` / `nˈɪvən` ("NIV-un") / `bˈɜːkɪl` ("BUR-kil") / `ˈɛdwɪn` / `bˈɪznɪsmɛn` / `ʧˈuː` ("choo", rhymes with "shoe") / `bˈɑːlɪk` ("BAH-lik") / `kuːmˈɑːri` ("koo-MAH-ree") / `nəhˈæpən` ("nuh-HAP-un") / `rəʤˈuː` ("ruh-JOO") / `ˌaɡɹɪhˌɔːtɪkˈʌlʧəɹəl` — all single-obvious-reading, no sample needed. `kˈʌpɪdʒ` ("CUP-ij", like cabbage — candidate 1 of 2, over the French-style "cup-AHZH") / `kˈɪlɪni` ("KILL-in-ee" — candidate 1 of 2, over "kil-INN-ee", the real Irish place name's actual stress) / `sˈiːə` ("SEE-ah" — candidate 3 of 3, over "SAY-ah" and the one-syllable "seh") / `kjˈæt` ("kyat", one syllable, like the currency — candidate 2 of 2, over two-syllable "kee-AHT") / `pəˈrɑːnəkən` ("puh-RAH-nuh-kan" — candidate 2 of 2, over the Malay-penultimate-stress "per-ah-NAH-kan"; recurs across many Singapore posts, worth remembering) / `ɡjˈɒk` ("gyock", one syllable — candidate 2 of 2, over two-syllable "ghee-OCK") / `kˈuːk` ("kook", the Dutch reading of "Koek" — candidate 1 of 2, over the anglicized two-syllable "KOH-ek"), all picked by Chris by ear (2026-09-10) from samples in `scratch/the-nutmeg-estates-that-became-orchard-road-pronunciation/` | `PRONUNCIATION_OVERRIDES` | the-nutmeg-estates-that-became-orchard-road |
| Elizabeth Choy post: the subject's own name "Choy"/"Choys" (×10+), her birth name "Su-Moi", place names "Kudat"/"Kadazan"/"Sandakan"/"Selegie"/"Miyako", her husband "Khun"/"Heng", "Mountbatten", "Baden-Powell", "MacKenzie", "Queenstown", "qipaos", "Jaywick" (Operation Jaywick) — **plus a plain English gap** "testified" and "Margaret's" (misaki lacks even plain "Margaret"); "King George VI"/"Elizabeth II" letter-spelling-fallback-on-the-wrong-token (same root cause as "King Edward VII") | Unknown word / Letter-spelling fallback applied to the wrong token | Spelled out letter by letter | `ʧˈY` ("choy", rhymes with "boy"/"joy") + `ʧˈYz` / `sˈuːmˌY` ("SOO-moy") / `kˈuːdat` ("KOO-dat") / `kɑːdˈɑːzan` ("kah-DAH-zan") / `sandˈɑːkan` ("san-DAH-kan") / `səlˈɛɡi` ("suh-LEH-gee", per a web summary of local usage as "sə-lə-gi") / `miːjˈɑːkQ` ("mee-YAH-koh") / `kˈuːn` ("koon") / `hˈɛŋ` (rhymes with "length" minus the "l") / `mˈWntbatᵊn` / `bˈAdᵊnpˌWᵊl` ("BAY-dn POW-el", confirmed via a pronunciation-dictionary source — misaki's own partial entry for "Baden" alone is wrong, reading it "BAH-dn") / `məkˈɛnzi` ("muh-KEN-zee") / `kwˈiːntWn` (compositional) / `ʧiːpˈWz` ("chee-POWZ" — Mandarin "qi" = the "chee" sound) / `ʤˈAwɪk` ("JAY-wick") / `tˈɛstɪfId` (misaki has "testify"/"testifies" but not this past-tense form) / `mˈɑːɡərəts` — all single-obvious-reading, no sample needed. Text expanded: `King George VI` → "King George the Sixth", `Elizabeth II` → "Elizabeth the Second" (two distinct royal figures in this post — the 1946 "Queen Elizabeth" she had a private audience with was the Queen Mother, King George VI's consort, not the future Elizabeth II, whose 1953 coronation she separately attended as a Legislative Council member). `ɡɔːdˈiːn` ("gor-DEEN", like "sardine" — the sculptor Dora Gordine, no definitive pronunciation source found, candidate 1 of 2, ear-picked by Chris from samples in `scratch/elizabeth-choy-canteen-operator-who-wouldnt-break-pronunciation/`) | `PRONUNCIATION_OVERRIDES` + `ABBREVIATION_EXPANSIONS` | elizabeth-choy-canteen-operator-who-wouldnt-break |
| Tiong Bahru / SIT post: "hoc" ("ad hoc"), the architect surname "Kan", "prewar", "dockworkers", the Hokkien nickname fragments "puay"/"chu" (*puay kee chu*, "aeroplane house") and "mei" (*mei ren wo*, "den of beauties") — **plus the post's own subject name "Tiong"** (as in Tiong Bahru, used 12+ times); and later, folded in during a post-approval edit on the lease-premium closing paragraph, the compounds "buyback" and "non-resettable" | Unknown word | Spelled out letter by letter | `hˈɒk` / `kˈɑːn` ("kahn") / `pɹˈiːwɔː` ("PREE-wor") / `dˈɒkwˈɜːkəz` / `ʧˈuː` ("choo", matches this dict's existing "Choo") / `mˈA` ("may") — all single-obvious-reading, no sample needed. `pwˈA` ("pway") for "puay" is a best guess, **NOT ear-verified yet** (single-mention colloquial nickname, low stakes). `tjˈɒŋ` ("tyong", literal spelling with a y-glide — candidate 2 of 3) picked by Chris by ear (2026-09-12) for "Tiong" itself, over `ʧˈɒŋ` ("chong", the anglicized Hokkien-glide reading a web summary of local usage had suggested) and `tiːˈɒŋ` ("tee-ong", fully spelled), from samples in `scratch/before-hdb-singapore-improvement-trust-tiong-bahru-pronunciation/`. `bˈIbak` ("BUY-back") and `nˌɒnɹiːsˈɛtəbᵊl` ("non-ree-SET-uh-bul") assembled from misaki's own existing "buy"/"back"/"reset"/"-able" (portable/notable/capable) fragments — both single-obvious-reading compounds, no sample needed. **Redone again after Chris watched the finished video (2026-09-12):** "puay kee chu" and "mei ren wo" aren't three equal-weight words each — "puay kee" (Hokkien "airplane") and "mei ren" (Mandarin "beautiful person") are each one word, with "chu" (house) and "wo" (nest/den) as the separate second word, a 2+1 grouping the original plain-space-separated overrides didn't convey (misaki/Kokoro gave all three tokens equal stress and gaps, so it read as three words). Fixed as a text-rewrite + fused-phoneme-override pair, not just a phoneme swap: two new `ABBREVIATION_EXPANSIONS` regexes join "puay kee" → "puay-kee" and "mei ren" → "mei-ren" before phonemization, and `PRONUNCIATION_OVERRIDES` now keys on those fused forms — `pwˈAkiː` ("PWAY-kee", one stress) and `mˈAɹɛn` ("MAY-ren", one stress) — replacing the old standalone "puay"/"mei" entries. Chris also separately flagged "wo" itself: misaki's silent default `wˈQ` ("woe", rhyming with "go") "came off as foreign" against the Mandarin word he actually knows; picked `wˈɔː` ("waw", rhyming with "war") from a 2-candidate ear-pick in `scratch/before-hdb-singapore-improvement-trust-tiong-bahru-redo/`. The "puay"/"pway" phoneme choice itself was already right (Chris: "it is what i wud choose") — only the grouping and "wo" needed fixing. Since these are text-level rewrites (not pure phoneme overrides on unchanged text), the sentence cache required a full `--no-cache` re-run of step 1.2 to actually pick up the fix; the new timing.json's offsets came out byte-identical to the old ones, so no video config changes were needed, just a re-render | `PRONUNCIATION_OVERRIDES` + `ABBREVIATION_EXPANSIONS` | before-hdb-singapore-improvement-trust-tiong-bahru |
| Whampoa post ("Whampoa: The Merchant Who Brought Singapore Its First Ice"): "Hoo" (Hoo Ah Kay), "Huangpu" (modern place name gloss), the closed compound "shipchandler", "Albemarle" (Henry Keppel's father's earldom), "Gilbert" (Gilbert Angus, ice-house partner), "Frederick" (Frederick Tudor, the ice trade's pioneer), "Bendemeer" — **plus the post's own subject name "Whampoa"**, used dozens of times | Unknown word | Spelled out letter by letter | `hˈuː` ("hoo", rhymes with "who") / `hwˈɑːŋpuː` ("hwahng-poo" — best guess, low stakes, single mention) / `ʃˈɪpʧɑːndlə` (misaki knows "ship" and "chandler" separately, not the closed compound) / `ˈalbəmɑːl` ("AL-buh-marl") / `ɡˈɪlbət` / `fɹˈɛdɹɪk` / `bˈɛndəmˈɪə` ("BEN-duh-MEER") — all single-obvious-reading, no sample needed. "Whampoa" itself (the post's own title/subject word) needed two rounds: the first 3 candidates (`wˈampoə` stress-first, `wampˈoə` stress-middle, `wˌampoˈɑː` stress-final) all carried a trailing schwa from the "poa" fragment; Chris caught that both his shortlisted picks "spells the rhyme correctly but both seems to include the a in the sound... what is typically spoken is WHAMpoh not WHAMpoah" (2026-09-12) — fixed by dropping the schwa entirely rather than re-picking among the original 3, giving 2 redone candidates (`wˈampQ` stress-first, `wampˈQ` stress-middle); Chris picked `wˈampQ` ("WHAM-poh"). Also: "CMG" (the honour Whampoa received) added to `KNOWN_LETTER_SPELLED` — read letter by letter, misaki's default is already correct, just not yet on the allowlist. Samples in `scratch/whampoa-the-merchant-who-brought-singapores-first-ice-pronunciation/` | `PRONUNCIATION_OVERRIDES` + `KNOWN_LETTER_SPELLED` | whampoa-the-merchant-who-brought-singapores-first-ice |
| Bumboats post ("The Bumboats and the River That Was Scrubbed Clean"): the boat-trade vocabulary "twakow"/"twakows", "tongkang"/"tongkangs", "ferried", "Chulia", "towkay"/"towkays" — plus plain-English closed-compound gaps "hometown", "secondhand", "nightsoil" — and place names "Peirce" (Upper Peirce Reservoir), "Lorong"/"Halus" (Lorong Halus), and "pok" (mee pok) — **plus "FX" and "HSBC"** (letter-spelling fallback, not yet allowlisted) | Unknown word / Letter-spelling fallback | Spelled out letter by letter | `twˈɑːkW` ("TWAH-kow") + `twˈɑːkWz` / `tˈɒŋkaŋ` ("TONG-kang") + `tˈɒŋkaŋz` / `fˈɛɹid` ("FERR-eed", rhymes with "buried") / `ʧˈuːliə` ("CHOO-lee-uh") / `tˈWkA` ("TOW-kay") + `tˈWkAz` / `hˈQmtWn` / `sˈɛkəndhand` / `nˈItsYl` / `pˈɪəs` ("peers/pierce") / `lɔːɹˈɒŋ` ("loh-RONG") / `hˈɑːluːs` ("HAH-loose") / `pˈɒk` ("mee pok", rhymes with "rock" not "poke") — all single-obvious-reading, no sample needed (well-established loanword/place-name pronunciations, not genuinely contested). "FX" and "HSBC" added to `KNOWN_LETTER_SPELLED` — read letter by letter, misaki's default is already correct | `PRONUNCIATION_OVERRIDES` + `KNOWN_LETTER_SPELLED` | the-bumboats-and-the-river-that-was-scrubbed-clean |
| Marina Barrage post ("Marina Barrage and the End of Singapore's Water-Dependency Anxiety"): plain-English closed-compound gaps "freshwater" and the brand name "NEWater" (PUB's reclaimed-water tap, styled as one word), plus Malay/Malaysian names from the water-agreement history — "Tebrau"/"Scudai" (the 1961 agreement's rivers), "Tunku"/"Rahman" (Tunku Abdul Rahman) | Unknown word | Spelled out letter by letter | `fɹˈɛʃwɔːtə` / `njˈuːwɔːtə` ("NEW-water") / `təbɹˈW` ("tuh-BROW") / `skuːdˈaɪ` ("skoo-DYE") / `tˈʊŋkuː` ("TOONG-koo") / `ɹɑːmˈɑːn` ("rah-MAHN") — all single-obvious-reading, no sample needed (none are this post's own repeated subject term) | `PRONUNCIATION_OVERRIDES` | marina-barrage-and-the-end-of-singapores-water-dependency-anxiety |
| P.S. Hunter post ("Peter Sinclair Hunter: The Health Officer Whose One Report Built Tiong Bahru"): the subject's own middle name "Sinclair", place/person names "Leonie" (Leonie Hill Road), "Swettenham" (Port Swettenham), "Bartley", "Middleton" (Middleton Hospital), the Latin mosquito-species name "ludlowi" (Anopheles ludlowi) — and later, folded in during a post-approval edit adding the "why it matters today" closer's note on modern dengue control, the bacterium genus "Wolbachia" (NEA's Project Wolbachia) | Unknown word | Spelled out letter by letter | `sˈɪŋkleə` ("SIN-klair") / `liˈQni` ("lee-OH-nee") / `swˈɛtənəm` ("SWET-n-um") / `lˈʌdlQaɪ` ("LUD-loh-eye" - Latin "-i" genitive read as "-eye" per scientific-Latin convention) / `bˈɑːtli` ("BART-lee") / `mˈɪdltən` ("MID-l-tun") / `wɒlbˈækiə` ("wol-BACK-ee-uh") — all single-obvious-reading, no sample needed (the post's own repeated subject surname, "Hunter", already phonemizes correctly with no override) | `PRONUNCIATION_OVERRIDES` | peter-sinclair-hunter-the-health-officer-whose-one-report-built-tiong-bahru |
| Linggiu Reservoir post ("Linggiu Reservoir: The Dam Singapore Built Inside Another Country to Secure Its Own Water"): Malay words "Tinggi" (Kota Tinggi) and "Sungai" ("river") confirmed via Wiktionary IPA, names "Muhyiddin"/"Yassin" (Muhyiddin Yassin, then-Johor Mentri Besar) and "Mentri" (Mentri Besar) with no confirmed IPA source found, plain-English compound gap "homegrown" — plus "Linggiu" itself, the post's own repeated subject name | Unknown word | Spelled out letter by letter | `tˈiŋɡi` ("TING-ghee", Wiktionary /ˈtiŋɡi/) / `sˈuŋaɪ` ("SOO-ngye", Wiktionary /ˈsuŋaj/ — distinct from "Sungei"'s existing `sˈʊŋaɪ` entry above, different spelling/vowel) / `mˌuːhiːjˈɪdɪn` ("moo-hee-YID-in", best-effort) / `jˈɑːsɪn` ("YAH-sin") / `mˈɛntri` ("MEN-tree") / `ˌhQmɡɹˈQn` (built the same way as "outgrown") — "Linggiu" had three candidates synthesized to `scratch/linggiu-reservoir-pronunciation/` (`lˈɪŋɡiu` "LING-ghee-oo", `lˈɪŋɡjuː` "LING-gyoo", `lɪŋɡˈiːu` "ling-GHEE-oo"); Chris picked candidate 1 minus its trailing "u" sound — `lˈɪŋɡi` ("LING-ghee", two syllables, no final "oo") | `PRONUNCIATION_OVERRIDES` | linggiu-reservoir-the-dam-singapore-built-inside-another-country-to-secure-its-own-water |
| Cold Storage post ("Cold Storage and the Frozen-Meat Trade"): plain-English compound gap "Centrepoint" (the mall, British spelling of "centre"), corporate names "Jardine"/"Matheson" (Jardine Matheson) and "Macrovalue" (its 2025 buyer), Hokkien/Malay names "Siong" (Sheng Siong) and "Joo"/"Chiat" (Joo Chiat) — plus letter-spelling-fallback tokens "DFI" and "CS" (DFI Retail Group, CS Fresh), correct as spelled, added to `KNOWN_LETTER_SPELLED` | Unknown word | Spelled out letter by letter | `sˈɛntəpɔɪnt` ("SEN-tuh-point") / `dʒˈɑːdɪn` ("JAR-din") / `mˈæθɪsən` ("MATH-ih-sun") / `mˈækroʊvæljuː` ("MAK-roh-val-yoo") / `dʒˈuː` ("joo") / `tʃˈiːət` ("chee-ət", confirmed reading /dʒuː tʃiːət/) — all single-obvious-reading, no sample needed. `sjˈɒŋ` ("syong") for "Siong" built by the same "-iong" analogy already confirmed by ear for "Tiong" (see the SIT/Tiong Bahru post above); not itself ear-verified, but low stakes (single mention, well-known brand) | `PRONUNCIATION_OVERRIDES` + `KNOWN_LETTER_SPELLED` | cold-storage-and-the-frozen-meat-trade |
| "mall" | Wrong entry / homograph collision | misaki's own lexicon gives ordinary "shopping mall" a short-u vowel ("mull"), not the standard "mawl" reading (rhymes with "ball"/"hall") | `mˈɔːl` ("mawl"). Caught by ear by Chris on this post's opening line ("even a shopping mall on Orchard Road") - not detectable by `--dry-run`'s automated scan, same root-cause class as "stung"/"graves" above | `PRONUNCIATION_OVERRIDES` | cold-storage-and-the-frozen-meat-trade |
| "reliably" | Correct phonemes, wrong acoustic rendering (root cause 5) | misaki gives "reliable"/"reliability" the correct "ry" (/aɪ/) vowel but "reliably" specifically gets a wrong short-ɪ, dropping the "-LY-" sound - heard by Chris as garbled, "like the speaker was tongue tied" | First tried `ɹɪlˈIəbli` (reusing "reliable"'s own correct vowel via `pipe.g2p()`, not guessed) - **did not fix it**. Chris re-listened and still heard it wrong, just less mangled: "releably", an "ee" sound where "eye" belongs. Same failure signature as the "live-in workers" case above (correct phonemes, Kokoro still mis-renders this specific vowel sequence) - a phoneme override doesn't reliably fix this class. Fixed instead by rewording the sentence to use "reliable" (confirmed correct) in place of "reliably": "the same basic idea — reliable, controlled cold — well beyond meat" | Reword the post (no working `PRONUNCIATION_OVERRIDES` fix found) | cold-storage-and-the-frozen-meat-trade |
| Tan Kim Seng post ("Tan Kim Seng and the Fountain Nobody Can Explain"): the Mandarin institution name "Ge" (Chong Wen Ge), the English firm "Handyside" (Andrew Handyside & Co.), the place name "Fullerton" (Fullerton Square), and the Hokkien name syllables "Beng" (Tan Beng Swee) and "Jiak" (Tan Jiak Kim) | Unknown word | Spelled out letter by letter | `ɡˈʌ` ("guh") / `hˈandisaɪd` ("HAN-dee-side") / `fˈʊlətən` ("FULL-uh-tun") — all single-obvious-reading, no sample needed. `bˈɛŋ` ("beng") and `dʒiˈɑk` ("jee-ahk") for "Beng"/"Jiak" — both candidate 1, confirmed by Chris by ear (2026-09-16) from samples in `scratch/tan-kim-seng-and-the-fountain-nobody-can-explain-pronunciation/` (beng-1/beng-2, jiak-1/jiak-2/jiak-3) | `PRONUNCIATION_OVERRIDES` | tan-kim-seng-and-the-fountain-nobody-can-explain |
| Cathay Building post ("The Cathay Building: Singapore's First Skyscraper"): the Malay honorific "Dato", the Chinese surname syllable "Tho" (Loke Wan Tho), plain English compound gaps "arthouse"/"Picturehouse", the Indian independence leader's name "Subhas"/"Chandra"/"Bose", and the Japanese wartime terms "Daitoa"/"Gekijo"/"Syonan" | Unknown word | Spelled out letter by letter | `dˈɑːtoʊ` ("DAH-toh") / `tˈoʊ` ("toh", hard T — same pattern as "Tock"/"Seng") / `ˈɑːthaʊs` / `pˈɪktʃəhaʊs` — all single-obvious-reading, no sample needed. `səbˈɑːs` / `tʃˈʌndrə` / `bˈoʊs` ("bohss", rhymes with "dose" — the standard English broadcast reading, not "bohz") for "Subhas"/"Chandra"/"Bose", and `daɪtˈoʊə` / `ɡˈɛkidʒˌoʊ` for "Daitoa"/"Gekijo" (Japanese 大東亜/劇場 romaji approximations) — all candidate 1, confirmed by Chris by ear (2026-09-16) from samples in `scratch/the-cathay-building-singapores-first-skyscraper-pronunciation/` (bose-1/bose-2, japanese-1/japanese-2). `ʃˈoʊnæn` ("SHOH-nan") for "Syonan" — also candidate 1, confirmed by ear (syonan-1/syonan-2 in the same folder), but worth flagging the reasoning: Syonan is the Kunrei-shiki romanization of Shōnan (昭南), the wartime name for occupied Singapore, where "Sy" represents the "Sh" sound, not an English "s" — caught by checking the origin-language pronunciation rather than guessing from the English spelling (per Chris's standing rule) | `PRONUNCIATION_OVERRIDES` | the-cathay-building-singapores-first-skyscraper |
| Botanic Gardens/rubber post ("The Botanic Gardens and the Rubber That Remade Malaya"): the English surname "Wickham", the botanical Latin term "brasiliensis" (Hevea brasiliensis), the place name "Kew" (Kew Gardens), Malay place names "Lintang"/"Asahan" (Bukit Lintang, Bukit Asahan), and the Hokkien/Teochew name syllables "Chay"/"Yan" (Tan Chay Yan) | Unknown word | Spelled out letter by letter | `wˈɪkəm` ("WICK-um") / `brəzˌɪliˈɛnsɪs` ("bruh-zil-ee-EN-sis") / `kjˈuː` ("kyoo") / `lɪntˈɑːŋ` ("lin-TAHNG") / `ˌɑːsəhˈɑːn` ("ah-suh-HAHN") — all single-obvious-reading, no sample needed. `tʃˈeɪ` ("chay", rhymes with "day") and `jˈɑːn` ("yahn") for "Chay"/"Yan" are best-guess candidates, **not yet ear-verified** — samples awaiting Chris's pick in `scratch/the-botanic-gardens-and-the-rubber-that-remade-malaya-pronunciation/` (chayyan-1/chayyan-2/chayyan-3) | `PRONUNCIATION_OVERRIDES` | the-botanic-gardens-and-the-rubber-that-remade-malaya |
...
**For a word with roots in a specific ethnicity/language, default to how
that language actually says it** (confirmed by Chris, 2026-09-04) - check
the origin language's real pronunciation (Wiktionary IPA, or a source
like Forvo.com for a native-speaker recording) rather than guessing from
the English spelling or from the standard romanization alone. The two
can genuinely diverge: "Kempeitai" (the wartime Japanese military
police) is a case in point - modern Hepburn romanizes 憲兵隊 as
"Kenpeitai" (with an "n"), but Japanese ん physically assimilates to
[m] before a bilabial consonant (p/b/m), so native speech actually
sounds closer to "kem-", which is exactly why the older English
military-history spelling is "Kempeitai." Surface this kind of tension
to Chris rather than picking silently - he chose "ken" for that one
anyway, matching the modern spelling over the more phonetically literal
reading, a legitimate call either way.
