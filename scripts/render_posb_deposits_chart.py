...
def render():
    ...
    foot_font = load_font(int(17 * SS), bold=False)
    ...
    # start
    dot(1877, 19865, 5)
    d.text((x_of(1877) + 12 * SS, y_of(19865) + 6 * SS),
           "1877: 211 depositors, $19,900", font=anno_font, fill=CHART_SECONDARY, anchor="lm")
    # the mid-century drift
    dot(1966, 37_400_000, 5)
    d.text((x_of(1966), y_of(37_400_000) + 34 * SS),
           "1957–66: a decade of drift", font=anno_font, fill=CHART_SECONDARY, anchor="mm")
    # the 1969 turn
    d.text((x_of(1969) - 12 * SS, y_of(57_700_000) - 16 * SS),
    ...
    d.text((ex - 12 * SS, ey - 34 * SS), "Transferred to DBS, 1998",
           font=anno_strong, fill=CHART_TEXT, anchor="rm")
    d.text((ex - 12 * SS, ey - 8 * SS), "$25.5 billion in deposits",
           font=anno_font, fill=CHART_SECONDARY, anchor="rm")

    d.text((margin, (H - 42) * SS), FOOT, font=foot_font, fill=CHART_MUTED)
