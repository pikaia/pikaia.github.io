...
- `date` includes a time (e.g. `2026-07-18 09:00:00 +0800`), not just a day, and **is the moment the post is created (now), not a future slot** (standing rule from 2026-09-24): posts go live as soon as they are pushed, so `date:` is already in the past when the file is first committed and the filename's date is today's. Do not future-date a post, and do not use `publish-early.sh` for new posts - it only exists for the older future-dated ones. Push the draft as soon as it is written (right after the pronunciation dry-run) so it can be reviewed on prod; later pipeline steps land as follow-up commits. `last_modified_at` starts equal to `date`. The homepage (`_layouts/home.html`) sorts posts by `last_modified_at` descending, so it reflects post/update recency, not just original publish date.
...
- **First-person asides** (the author's own memory or connection to the subject) are written as plain prose in the first person — no italics, no blockquote, no callout box; the switch to "I" is the only signal needed. Each aside gets **its own paragraph**, not tacked onto the end of a third-person narration paragraph. This applies whether first person is the whole post's voice (e.g. the Kangkar post) or a single mid-post aside (e.g. the Nee Soon paragraph in the pineapple-kings post).
...
**Do not click through NLB's own full-page image viewer from the browse-by-date index, on any page.** Clicking a page thumbnail from that index hits a Terms of Use gate ("I Agree" / "Cancel") whose text states SPH Media/Mediacorp content should be obtained by formally requesting it from NLB — a signal they don't intend their own scans for casual reuse, even for pre-1987 editions where the underlying copyright has expired. Never accept that agreement on the user's behalf. This restriction is about NLB's own platform terms, not a claim that the content itself is still in copyright — Cap. 63 §96 (below) covers any page of a qualifying edition equally, front page or not.

**Correction (2026-09-05):** that gate is triggered by the click path from the browse-by-date index, not by the underlying page itself. A direct URL to a full page — `https://eresources.nlb.gov.sg/newspapers/digitised/page/<issue-id>-1.1.<page>` (found via NewspaperSG's own search, a Google search, or an article-level URL's breadcrumb) — loads the clean, full-resolution, ungated page scan directly, no agreement modal, confirmed by scrolling one end to end. Since there's no modal to accept, the "never accept on the user's behalf" constraint doesn't block this route — it was specifically about not clicking "I Agree" for the user, and there's nothing to click here. This is still the same Cap. 63 §96 legal basis below (full page, pre-1987 edition), just a cleaner way to actually get the scan.
...
**Using a pre-1956 Straits Times page as an image (first done 2026-09-21, the 1915 Mutiny post).** For an issue well inside SPH's 70-year line (anything before 1956; check the current date's threshold), the whole-page scan can be used as a post/gallery image. Get it from the ungated direct page route (`https://eresources.nlb.gov.sg/newspapers/digitised/page/<issue-id>-1.1.<page>`; the image itself is served at `https://eservice.nlb.gov.sg/newspapercontent/digitised/page/<issue-id>-1.1.<page>.webp?size=5`, found via the read_network_requests tool), save it as a JPEG in `assets/images/straits-times-YYYY-MM-DD-page-N.jpg` (a documented exception to the no-committed-binaries rule, like the OSM tile), and use it in a float. Always the **whole page, never a crop of one article** (a request to "clip the article" is answered with the whole page). Caption credit: "Singapore Press Holdings, via NewspaperSG, National Library Board Singapore, public domain". Articles behind the terms gate can also be read by asking Chris to click "I Agree" himself, but the direct page route avoids the gate entirely; either way, use the text for facts and cite the headline/date/page in Sources.
